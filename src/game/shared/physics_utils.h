...
EXPORT_GAME_SHARED pvector<BulletRayHit> ray_test_all_stored( const LPoint3 &from, const LPoint3 &to,
					   const BitMask32 &mask = BitMask32::all_on() );
EXPORT_GAME_SHARED void ray_test_closest_not_me( RayTestClosestNotMeResult_t &result,
...
EXPORT_GAME_SHARED void attach_bullet_nodes( const NodePath &root_node );
EXPORT_GAME_SHARED void remove_bullet_nodes( const NodePath &root_node );
EXPORT_GAME_SHARED void detach_bullet_nodes( const NodePath &root_node );

EXPORT_GAME_SHARED void detach_and_remove_bullet_nodes( const NodePath &root_node );

EXPORT_GAME_SHARED LVector3 get_throw_vector( const LPoint3 &trace_origin, const LVector3 &trace_vector,
