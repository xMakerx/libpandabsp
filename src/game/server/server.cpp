...
void Server::run_networking_events()
{
...
void Server::remove_entity( CBaseEntity *ent )
{
...
PT( CBaseEntity ) Server::make_player()
{
...
{
	( (BaseGame *)g_game )->run_cmd( cmd, cl );
}
...
PT( CBaseEntity ) Server::make_entity_by_name( const std::string &name, bool spawn,
...
void Server::send_tick( Client *cl )
{
