...
static CClientGlobalVars client_globals( true );
...
{
	g_globals = &client_globals;
	g_globals->game = this;
}

void C_BaseGame::run_cmd( const std::string &full_cmd )
{
...
void C_BaseGame::start_game( const std::string &mapname )
{
...
void C_BaseGame::do_frame()
{
	...
	g_globals->realtime = clock->get_real_time();
	g_globals->curtime = clock->get_frame_time();
	g_globals->frametime = clock->get_dt();
	g_globals->absoluteframetime = g_globals->frametime;
	g_globals->framecount = clock->get_frame_count();
...
bool C_BaseGame::startup()
{
	...
	if ( fps_max > 0 )
	{
		...
		global_clock->set_mode( ClockObject::M_limited );
		global_clock->set_frame_rate( fps_max );
	}
...
DEFINE_TASK_FUNC( C_BaseGame::client_task )
{
	C_BaseGame *self = (C_BaseGame *)data;
	self->_client->tick();
...
DEFINE_TASK_FUNC( C_BaseGame::entities_task )
{
	C_BaseGame *self = (C_BaseGame *)data;
...
DEFINE_TASK_FUNC( C_BaseGame::cmd_task )
{
	C_BaseGame *self = (C_BaseGame *)data;
