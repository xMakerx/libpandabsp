...
extern const struct LibraryDef libpandabsp_moddef;
extern void Dtool_libpandabsp_RegisterTypes();
extern void Dtool_libpandabsp_BuildInstants(PyObject *module);

#if PY_MAJOR_VERSION >= 3
extern "C" EXPORT_CLASS PyObject *PyInit_libpandabsp();
#else
extern "C" EXPORT_CLASS void initlibpandabsp();
#endif

#if PY_MAJOR_VERSION >= 3
static struct PyModuleDef py_libpandabsp_module = {
...
PyObject *PyInit_libpandabsp() {
  PyImport_Import(PyUnicode_FromString("panda3d.core"));
  PyImport_Import(PyUnicode_FromString("panda3d.bullet"));
  Dtool_libpandabsp_RegisterTypes();

  const LibraryDef *defs[] = {&libpandabsp_moddef, nullptr};

  PyObject *module = Dtool_PyModuleInitHelper(defs, &py_libpandabsp_module);
  if (module != nullptr) {
    Dtool_libpandabsp_BuildInstants(module);
  }
  return module;
}
...
void initlibpandabsp() {
  ...
  PyObject *module = Dtool_PyModuleInitHelper(defs, "libpandabsp");
  if (module != nullptr) {
    Dtool_libpandabsp_BuildInstants(module);
  }
}
