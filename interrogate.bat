...
%INTERROGATE% -fnames -string -refcount -assert -python-native -S%PANDA_INCLUDE% -S%PANDA_INCLUDE%/parser-inc -I./include -srcdir ./include -oc src/%MODULE%_igate.cpp -od src/%MODULE%.in -module %MODULE% -library %MODULE% -Dvolatile= -DINTERROGATE -DWITHIN_PANDA -DWITHINPANDA -DHAVE_NET -DCPPPARSER -DCIO -D__STDC__=1 -D__cplusplus=201103L -D__inline -D_X86_ -DWIN32_VC -DWIN32 -D_WIN32 -D_MSC_VER=1600 -D"__declspec(param)=" -D__cdecl -D_near -D_far -D__near -D__far -D__stdcall config_bsp.h bsploader.h entity.h bsp_render.h shader_generator.h shader_spec.h bsp_material.h TexturePacker.h shader_vertexlitgeneric.h shader_lightmappedgeneric.h shader_unlitgeneric.h shader_unlitnomat.h shader_csmrender.h raytrace.h shader_skybox.h ambient_boost_effect.h audio_3d_manager.h ciolib.h bounding_kdop.h shader_decalmodulate.h glow_node.h postprocess/postprocess.h postprocess/hdr.h postprocess/bloom.h lighting_origin_effect.h planar_reflections.h postprocess/fxaa.h bloom_attrib.h physics_character_controller.h
